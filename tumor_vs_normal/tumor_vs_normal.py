...
if __name__ == '__main__':
    ...
    for utr3 in utr3s:
        ...
        logging.debug('nclusters = {}'.format(lclusters))
        if lclusters > args.max_sites:
            logging.debug('Number of clusters exceeds maximum allowed, skipping')
            continue
        mtx = Matrix(2, lclusters)
        centroids = []
        ...
        for i, _type in enumerate(tkeys):
            ...
            for j in xrange(mtx.n):
                ...
                score = len(values)
                if args.normalize:
                    score /= float(expression_mean * nsamples)
                mtx.mtx[i][j] = score
        if skip:
            continue
        mtx.rownames = tkeys
        if utr3.strand == '+':
            mtx.colnames = ['c_{}_{}'.format(int(x), int(100*(x - utr3.start)/(lutr3))) for x in centroids]
        else:
            mtx.colnames = ['c_{}_{}'.format(int(x), int(100*(utr3.end - x)/(lutr3))) for x in centroids]
